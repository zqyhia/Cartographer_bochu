...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 10 -- Ceres 扫描匹配器的平移权重为 2e2 10
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 40 -- 旋转惩罚权重。越高说明越相信运动模型。40
TRAJECTORY_BUILDER_2D.pose_extrapolator.imu_based.pose_translation_weight = 2.0 -- 基于 IMU 进行位姿外推的位移权重参数
TRAJECTORY_BUILDER_2D.pose_extrapolator.imu_based.pose_rotation_weight = 2.0 -- 基于 IMU 进行位姿外推的旋转权重参数
-- TRAJECTORY_BUILDER_2D.pose_extrapolator.imu_based.odometry_translation_weight = 0.3 -- 基于 里程计 进行位姿外推的旋转权重参数
-- TRAJECTORY_BUILDER_2D.pose_extrapolator.imu_based.odometry_translation_weight = 0.3 -- 基于 里程计 进行位姿外推的旋转权重参数


-- 位姿图优化配置
POSE_GRAPH.constraint_builder.min_score = 0.5 -- 约束构建器的最小得分为 0.65
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.6 -- 全局定位的最小得分为 0.7
...
POSE_GRAPH.optimize_every_n_nodes = 20 -- 每 50 个节点优化一次位姿图（纯定位改为20)
