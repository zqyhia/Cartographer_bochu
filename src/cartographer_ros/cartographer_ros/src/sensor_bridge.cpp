...
SensorBridge::SensorBridge(
    ...
    const double lookup_transform_timeout_sec, tf2_ros::Buffer* const tf_buffer, // 前四个参数是配置文件中的参数
    carto::mapping::TrajectoryBuilderInterface* const trajectory_builder)
    // 第五个参数trajectory_builder是将sensor_bridge对象转换后数据传给Cartographer的接口，通过map_builder_->GetTrajectoryBuilder(trajectory_id)获得
    : num_subdivisions_per_laser_scan_(num_subdivisions_per_laser_scan),
      tf_bridge_(tracking_frame, lookup_transform_timeout_sec, tf_buffer), // tf_bridge_：cartographer_ros中关于ROS坐标变换的封装
      trajectory_builder_(trajectory_builder) {}

// ToOdometryData：将ROS的Odometry消息(nav_msgs::msg::Odometry)转换为Cartographer内部格式的里程计数据
...
// HandleOdometryMessage：将ROS格式的里程计消息转换为Cartographer内部格式并传递给SLAM系统
...
// 将 ROS IMU 消息转换为 Cartographer 内部格式
...
// HandleLaserScanMessage：Cartographer ROS 2 接口中处理激光扫描(LaserScan) ROS 消息的入口方法，负责将 ROS 激光扫描消息转换为 Cartographer 内部格式并转发处理
...
// HandleLaserScan：将原始激光扫描数据转换为适合SLAM系统处理的格式
/*
检查输入数据有效性
将激光扫描数据分割为多个子分区(subdivision)
处理每个子分区的时间戳
最终调用HandleRangefinder处理转换后的数据
*/
...
// HandleRangefinder：Cartographer 中处理测距仪(rangefinder)数据的核心方法，负责将点云数据转换并传递给 SLAM 系统的轨迹构建器
