...
def generate_launch_description():
    ## ***** 声明参数 *****
    bag_filename_arg = DeclareLaunchArgument('bag_filename') # 声明bag_filename参数，用于指定rosbag2文件的路径

    ## ***** 节点 *****
    # 启动其他launch文件：cartographer_ros包的共享目录路径/launch/fishbot_2d.launch.py
    wheeltec_2d_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(FindPackageShare('cartographer_ros').find('cartographer_ros') + '/launch/wheeltec_2d_location.launch.py'),
    ...
    rviz_node = Node(
        ...
        arguments = ['-d', FindPackageShare('cartographer_ros').find('cartographer_ros') + '/configuration_files/wheeltec_2d.rviz'], # '-d'用于指定RViz2的配置文件，使用cartographer_ros为RViz2 2d demo写好的配置文件
        parameters = [{'use_sim_time': True}], # 传递给节点的参数，表示 使用仿真时间
    )

    # 播放rosbag2（使用外部命令行:ros2 bag play [bag_filename] --clock）
    ros2_bag_play_cmd = ExecuteProcess(
        cmd = ['ros2', 'bag', 'play', LaunchConfiguration('bag_filename'), '--clock'], # --clock会发布模拟时间话题/clock
        name = 'rosbag_play',
    )
