...
class TrajectoryBuilderInterface {
 public:
  struct InsertionResult { // InsertionResult 就是用来保存插入 Local Slam 的一个节点的数据结构
    NodeId node_id; // NodeId 是一个结构体，在/mapping/id.h 中定义，包含两个部分：一个 int 型的 trajectory_id 和一个 int 型的 node_index.
    std::shared_ptr<const TrajectoryNode::Data> constant_data; // TrajectoryNode::Data 包含了经过处理的一帧传感器数据，TrajectoryNode 是一个结构体，在/mapping/trajectory_node.h 中定义，
    std::vector<std::shared_ptr<const Submap>> insertion_submaps; // insertion_submaps：已经建立起来的子图列表（这里有点奇怪，一帧传感器数据竟然对应多个子地图？）
  };
