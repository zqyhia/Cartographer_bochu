...
class GlobalTrajectoryBuilder : public mapping::TrajectoryBuilderInterface {
 public:
  ...
  void AddSensorData(
    ...
    if (local_slam_result_callback_) { // 检查回调函数是否被注册，如果注册了，就移动数据到后端
  ...
  // CollatedTrajectoryBuilder构建函数里会调用这个
 ...
 private:
  ...
  std::unique_ptr<LocalTrajectoryBuilder> local_trajectory_builder_; // LocalTrajectoryBuilder2D的指针
