...
// AddTrajectoryIfNeeded：确保指定轨迹ID的基础数据结构已初始化，并验证轨迹状态的合法性
void PoseGraph2D::AddTrajectoryIfNeeded(const int trajectory_id) {
  data_.trajectories_state[trajectory_id]; // 轨迹状态容器隐式初始化​
  CHECK(data_.trajectories_state.at(trajectory_id).state != // 轨迹状态校验：轨迹不能是FINISHED状态
        TrajectoryState::FINISHED);
  CHECK(data_.trajectories_state.at(trajectory_id).state != // 轨迹状态校验：轨迹不能是DELETED状态
        TrajectoryState::DELETED);
  CHECK(data_.trajectories_state.at(trajectory_id).deletion_state == // 删除状态校验：防止在轨迹即将被删除时修改其数据
        InternalTrajectoryState::DeletionState::NORMAL);
  data_.trajectory_connectivity_state.Add(trajectory_id); // 连接性状态初始化：维护轨迹间的拓扑连接关系，用于闭环检测
  // Make sure we have a sampler for this trajectory.
  if (!global_localization_samplers_[trajectory_id]) { // 全局定位采样器初始化：控制全局定位（闭环检测）的计算频率
...
// AddSubmapFromProto：根据解析出的proto内容添加子地图
void PoseGraph2D::AddSubmapFromProto(
    const transform::Rigid3d& global_submap_pose, const proto::Submap& submap) {
  if (!submap.has_submap_2d()) { // 如果不是2D子地图，直接返回
    return;
  }

  const SubmapId submap_id = {submap.submap_id().trajectory_id(), // 提取子地图ID
                              submap.submap_id().submap_index()};

  const transform::Rigid2d global_submap_pose_2d = // 位姿降维处理
      transform::Project2D(global_submap_pose);
  {
    absl::MutexLock locker(&mutex_);
    const std::shared_ptr<const Submap2D> submap_ptr = // 子地图对象构造​
        std::make_shared<const Submap2D>(submap.submap_2d(),
                                         &conversion_tables_); // conversion_tables_用于概率值与网格值的转换
    AddTrajectoryIfNeeded(submap_id.trajectory_id); // 轨迹存在性检查​
    if (!CanAddWorkItemModifying(submap_id.trajectory_id)) return;
    data_.submap_data.Insert(submap_id, InternalSubmapData()); // 子地图数据插入​
    data_.submap_data.at(submap_id).submap = submap_ptr;
    // Immediately show the submap at the 'global_submap_pose'.立即在‘global_submap_pose’中显示子地图。
...
void PoseGraph2D::AddTrimmer(std::unique_ptr<PoseGraphTrimmer> trimmer) {
  ...
  PoseGraphTrimmer* const trimmer_ptr = trimmer.release(); // 释放unique_ptr所有权（C++11 lambda限制）
  AddWorkItem([this, trimmer_ptr]() LOCKS_EXCLUDED(mutex_) { // 创建工作项添加到队列
    absl::MutexLock locker(&mutex_); // 获取互斥锁保证线程安全
    trimmers_.emplace_back(trimmer_ptr); // 将Trimmer添加到容器
    return WorkItem::Result::kDoNotRunOptimization; // 返回不需要触发优化
  });
}
