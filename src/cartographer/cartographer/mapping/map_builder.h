...
class MapBuilder : public MapBuilderInterface {
 ...
 private:
  ...
  std::unique_ptr<PoseGraph> pose_graph_; // 一个 PoseGraph 的智能指针，用于loop closure

  std::unique_ptr<sensor::CollatorInterface> sensor_collator_; //收集传感器数据的智能指针
  std::vector<std::unique_ptr<mapping::TrajectoryBuilderInterface>>
      trajectory_builders_; //一个向量，管理所有的 TrajectoryBuilderInterface;应该是每一个 trajectory 对应了该向量的一个元素
